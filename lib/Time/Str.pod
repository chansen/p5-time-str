...
(F) A named parameter is outside its valid range.

=item Parameter '%s' is unknown: '%s'

(F) An unrecognised format specifier or named parameter was given.

=item Unable to parse: %s

(F) C<str2date> or C<str2time> could not parse the input string in the
specified format.
...
=item * ISO 8601:2019 - Data elements and interchange formats

=item * RFC 3339 - Date and Time on the Internet: Timestamps

=item * RFC 2822 / RFC 5322 - Internet Message Format

=item * RFC 2616 / RFC 7231 - HTTP/1.1

=item * RFC 4287 - The Atom Syndication Format

=item * RFC 5280 - Internet X.509 Public Key Infrastructure (PKIX)

=item * RFC 5545 - Internet Calendaring and Scheduling (iCalendar)

=item * RFC 9051 - Internet Message Access Protocol (IMAP)

=item * RFC 9557 - Date and Time on the Internet: Timestamps with Additional Information

=item * ISO 9075 - SQL Database Language

=item * ITU-T X.680 (ISO/IEC 8824-1) - ASN.1

=item * W3C Date and Time Formats
