...
X.509 certificate validity times as defined in RFC 5280 section 4.1.2.5.
Certificates encode validity as ASN.1 UTCTime (two-digit year) for dates
before 2050 and GeneralizedTime (four-digit year) for dates from 2050
onward. Both forms require UTC and do not permit fractional seconds or
timezone offsets.
...
month names, Roman numerals, and ordinal suffixes are all case-insensitive.
...
Roman numerals (C<I> through C<XII>) are accepted only in
