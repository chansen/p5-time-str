...
%%{
  ...
  ZoneOffsetBasic    = [+\-] digit{2} (digit{2})?;
  ZoneOffsetExtended = [+\-] digit{2} (':' digit{2})?;
  ...
  (
    (
      ...
      ( 'T' digit{2} >mark %set_hour ( ':' digit{2} >mark %set_minute
                                     ( ':' digit{2} >mark %set_second)?)?

        ([.,] digit{1,9} >mark %set_fraction)?
        (
             ZoneOffsetExtended >mark %set_tz_offset
           | 'Z'                >mark %set_tz_utc
        )?
      )?
    )
  |
    (
      ...
      ( 'T' digit{2} >mark %set_hour  (digit{2} >mark %set_minute
        ...
        ([.,] digit{1,9} >mark %set_fraction)?
        (
             ZoneOffsetBasic >mark %set_tz_offset
           | 'Z'             >mark %set_tz_utc
        )?
      )?
    )
